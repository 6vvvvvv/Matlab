...
% z=[z1,z2,z3]';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
A1=1.0;beta1=0.5;n1=95;gama1=0.5;%%%%%%%%%%%%非线性环节参数
...
%     dv(j)=(t/6)*(kk1+2*kk2+2*kk3+kk4);%%%%感觉好像不对
end
z2(:,i)=z2(:,i-1)+dv*t;
...
fprintf('结果矩阵，第一列为楼层数,第二列为Dy，第三列为无控位移最大，第四列为无控加速度最大，第五列为LQR位移最大，第六列为LQR加速度最大,第七列为非光滑位移最大，第八列为非光滑加速度最大')
table1
fprintf('非光滑最大控制力')
